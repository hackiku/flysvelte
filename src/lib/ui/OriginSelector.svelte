...
<script lang="ts">
  ...
  export let options: string[] = ['Sea Level', 'Center of Gravity', 'Earth Center'];
  export let selectedOption: Writable<string> = writable(options[0]);

  let dropdownOpen: Writable<boolean> = writable(false);
